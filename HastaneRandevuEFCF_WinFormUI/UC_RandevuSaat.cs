    ...
    public partial class UC_RandevuSaat : UserControl
    {
        ...
        private void comboBoxSaatler_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //Buttonların textleri ayarlanacak
                    ButonlarinTextleriniAyarla();
                    //Buttonların aktif pasif durumları ayarlanacak
                    DrveTrheGoreButonlarinAktifPasifIslemleriniYap();
                }
                else
                {
        ...
        private void DrveTrheGoreButonlarinAktifPasifIslemleriniYap()
        {
            try
            {
                ...
                if (Doktorum != null)
                {
                    ...
                    {
                        //Randevuları tek tek dolaşıp pasifleştirme işlemi yapalım
                        foreach (RandevuBilgileri item in drRandevulari)
                        {
                            if (item.RandevuTarihi.ToShortTimeString().Substring(0,2)==comboBoxSaatler.Text.Substring(0,2))
                            {
                                ...
                                string itemdk = itemsaatim.Substring(3, 2);  //sadece dk'yı aldık
        ...
        private void ButonlarinTextleriniAyarla()
        {
            if (comboBoxSaatler.SelectedIndex>=0)
            {
                string saatim = SaatViewModel.SaatlerListesiniGetir().FirstOrDefault(x => x.Id == 
                (int)comboBoxSaatler.SelectedValue).SaatDilimi.Substring(0,2);  //09 
                foreach (Button item in tableLayoutPanelRandevuButonlar.Controls)
                {
